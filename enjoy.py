...
def main():  # noqa: C901
    ...
    parser.add_argument("-f", "--log-folder", help="Log folder", type=str, default="rl-trained-agents")
    parser.add_argument("--algo", help="RL Algorithm", default="ppo", type=str, required=False, choices=list(ALGOS.keys()))
    parser.add_argument("-n", "--n-eval-steps", help="Number of evaluation timesteps", default=1000, type=int)
    ...
    parser.add_argument('--deterministic', help="1: Use deterministic actions, 0: Use stochastic actions", type=int, choices=[0, 1], default=0)
    parser.add_argument(
    ...
    parser.add_argument('--log-info', help="1: Log information at each evaluation steps and save, 0: don't log", type=int, choices=[0, 1], default=0)
    ...
    for t in range(args.n_eval_steps):
        ...
        if "widowx" in env_id:
            ...
            episode_success_list_50 = calc_ep_success(success_threshold_50, episode_success_list_50, infos)
            episode_success_list_20 = calc_ep_success(success_threshold_20, episode_success_list_20, infos)
            episode_success_list_10 = calc_ep_success(success_threshold_10, episode_success_list_10, infos)
            episode_success_list_5 = calc_ep_success(success_threshold_5, episode_success_list_5, infos)
            episode_success_list_2 = calc_ep_success(success_threshold_2, episode_success_list_2, infos)
            episode_success_list_1 = calc_ep_success(success_threshold_1, episode_success_list_1, infos)
            episode_success_list_05 = calc_ep_success(success_threshold_05, episode_success_list_05, infos)
        ...
        if args.plot_dim == 2:
            ...
            ax2.plot(tip[1], tip[2], marker='x', color='r', linestyle='', markersize=10, mew=3)
            ...
            ax.cla()
            ax.plot([goal[0]], [goal[1]], zs=[goal[2]], marker='o', color='g', linestyle='', markersize=10, alpha=0.5)
            ax.plot([tip[0]], [tip[1]], zs=[tip[2]], marker='x', color='r', linestyle='', markersize=10, mew=3)
            ax.set_xlim([-0.2, 0.2])
            ax.set_ylim([-0.2, 0.2])
            ax.set_zlim([0, 0.5])
        ...
        if args.n_envs == 1:
            ...
            if done and args.verbose > 0:
                ...
                if args.log_info:
                    ...
                    log_df.to_csv(log_path+"/res_episode_"+str(episode_nb)+".csv", index=False)  # slow
    ...
    if args.verbose > 0 and len(episode_rewards) > 0:
        ...
        if "widowx" in env_id:
            SR_mean_50, RT_mean_50 = calc_mean_successratio_reachtime(success_threshold_50, success_list_50, reachtime_list_50)
            SR_mean_20, RT_mean_20 = calc_mean_successratio_reachtime(success_threshold_20, success_list_20, reachtime_list_20)
            SR_mean_10, RT_mean_10 = calc_mean_successratio_reachtime(success_threshold_10, success_list_10, reachtime_list_10)
            SR_mean_5, RT_mean_5 = calc_mean_successratio_reachtime(success_threshold_5, success_list_5, reachtime_list_5)
            SR_mean_2, RT_mean_2 = calc_mean_successratio_reachtime(success_threshold_2, success_list_2, reachtime_list_2)
            SR_mean_1, RT_mean_1 = calc_mean_successratio_reachtime(success_threshold_1, success_list_1, reachtime_list_1)
            SR_mean_05, RT_mean_05 = calc_mean_successratio_reachtime(success_threshold_05, success_list_05, reachtime_list_05)
